...
public class Top10CountryAndYear {
	String year, country;
	public Top10CountryAndYear(String year, String country)
	{
		this.year = year;
		this.country = country;
	}
	
	public void top10CountryYearQueryProcess() throws IOException
	{
		String directory = "MyDatabases/Top10CountryYear";
		...
		String query_string = "PREFIX rat:<http://utdallas/semclass/vocabulary/rate#> "
				...
				+ "?uid usr:country ?ucountry. filter regex(str(?ucountry),'"+country+"','i')"
				+ "?rate rat:rate ?r.} "
				+ "group by (?bid) }} "
				+ "order by desc(?avg) "
				+ "limit 10"; 
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter("data/country.csv"));
